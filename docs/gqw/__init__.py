...
from conformance.diopi_functions import fill_, softmax, relu, abs
...
#         考虑一个形状为 :math:`(N, C, *)` 的 :attr:`input` 输入张量,
#         其中 :math:`N` 为批大小, :math:`C` 为通道数, :math:`*` 代表其空间维度。
#         该算子对于每一个卷积核覆盖下的区域, 元素会被重新排成一列。当卷积核在整个图片上滑动时, 将会形成一系列的列向量。
#         :attr:`output` 输出张量形状为 :math:`(N, C \times \prod(\text{kernel_size}), L)`, 其中
#         :math:`C \times \prod(\text{kernel_size})` 是块大小 (一个块有 :math:`\prod(\text{kernel_size})`
#         空间位置且包含 :math:`C` 个通道), :math:`L` 为块的数量。
...
#     参数
#         - *input* ( **Tensor** ): 输入张量
#         - *kernel_size* ( **int** 或者 **tuple** ): 卷积核大小
#         - *stride* ( **int** 或者 **tuple** ): 步长大小, 默认为 1
#         - *padding* ( **int** 或者 **tuple**, 可选): 输入两边隐式零填充, 默认为 0
#         - *dilation* (**int** 或者 **tuple**, 可选): 卷积核元素之间的间距, 默认为 1
#     形状
#         - *input* : 形状为 :math:`(N, C, *)`
#         - *output* : 形状为 :math:`(N, C \times \prod(\text{kernel_size}), L)`
#     C API
#         :guilabel:`diopiIm2Col`
#     """)
...
#         考虑一个形状为 :math:`(N, C \times  \prod(\text{kernel_size}), L)` 的 :attr:`input` 输入张量,
#         其中 :math:`N` 是批大小, :math:`C \times \prod(\text{kernel_size})`
#         是块大小 (一个块有 :math:`\prod(\text{kernel_size})`
#         空间位置且包含 :math:`C` 个通道), :math:`L` 为块的数量。 该算子通过对重叠部分值求和组合这些局部块为一个大的 :attr:`output` 输出张量,
#         其形状为 :math:`(N, C, \text{output_size}[0], \text{output_size}[1], \dots)`。需满足以下关系：
...
#     参数
#         - *input* ( **Tensor** ) : 输入张量
#         - *output_size* ( **int** 或者 **tuple** ) : 输出尺寸
#         - *kernel_size* ( **int** 或者 **tuple** ) : 卷积核大小
#         - *stride* ( **int** 或者 **tuple** ) : 步长大小, 默认为 1
#         - *padding* ( **int** 或者 **tuple**, 可选) : 输入两边隐式零填充, 默认为 0
#         - *dilation* (**int** 或者 **tuple**, 可选) : 卷积核元素之间的间距, 默认为 1
#     形状
#         - *input* : 形状为 :math:`(N, C \times \prod(\text{kernel_size}), L)`
#         - *output* :  形状为 :math:`(N, C, \text{output_size}[0], \text{output_size}[1], \dots)`
#     C API
#         :guilabel:`diopiCol2Im`
#     """)
...
#             A = LL^{\text{H}}   \qquad L \in \mathbb{K}^{n \times n}

#         其中 :math:`L` 为下三角矩阵,  :math:`L^{\text{H}}` 为其转置。
#     参数
#         - *input* ( **Tensor** ) : 形状为 `(*, n, n)` 的输入矩阵, 其中 `*` 为 0 或者批大小
#         - *upper* ( **bool**, 可选 ) : 是否返回上三角矩阵（ 即 :math:`L^{\text{H}}` ) , 默认为 False
#         - *check_errors* ( **bool**, 可选 ) : 是否检查错误产生, 默认为 False
#     C API
#         :guilabel:`diopiCholesky`
#     """)
...
#         如果 :attr:`A` 对角线包含0或者接近0 且 :attr:`unitriangular`\ `= False` (默认) 或者对于某些输入矩阵,
#         结果可能包含 `NaN` 。
#     参数:
#         - *input* ( **Tensor** ) : 方程右边, 形状为 :math:`(*, m, k)` 其中 `*` 为 0 或者批大小
#         - *A* ( **Tensor** ) : 上(下)三角系数矩阵, 形状为 :math:`(*, m, m)` 其中 `*` 为 0 或者批大小
#         - *upper* ( **bool**, 可选 ) : 是否求解上三角系数矩阵，或者求解下三角系数矩阵，默认为 True
#         - *transpose*  ( **bool**, 可选 ) : 是否转置 :math:`A` , 默认为 False
#         - *unitriangular* ( **bool**, 可选 ) : 是否 :math:`A` 是单位三角矩阵, 如果为 True, :math:`A` 的对角线元素被认为 1 而不是其实际值。默认为 False
#     返回值:
#         - *solution* ( **Tensor** ) : 方程 :math:`AX = b` 的计算结果 :math:`X`
#         - *cloned_coefficient* ( **Tensor** ) : :math:`A` 的拷贝
#     C API
#         :guilabel:`diopiTriangularSolve`
#     """)
