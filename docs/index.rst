...
DeepLink着力降低算力使用门槛，打破算力和框架壁垒
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

| DeepLink是一套设备无关的接口体系，作为 :red:`芯片于深度学习框架适配的“桥梁”，根本性实现软硬件解耦，破除生态壁垒`。遵守此标准可以实现主流框架与芯片高效适配，极大降低算力使用门槛，减少技术阻力。从而打破算力和框架的垄断，实现算力要素多样化。并且通过编译的力量，提升整体的训练效率。面对国际形势诡谲多变的现状，为发展国产替代、实现人工智能的软硬件独立自主起到了关键的纽带作用。
